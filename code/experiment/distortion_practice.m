% Function to run practice trials with unflanked letter stimuli
% before running the letter distortion detection experiment
% 
% params: 
% - subject code:   identification of the observer

function distortion_practice()
%% params
session = 1;
...
imPath = fullfile(filepathToHere, '../stimuli/practiceimg/distorted');
...
flankedtype = 'unflanked'; % test only unflanked images in practice trials
flanked = 0;
distortiontype = 'bex'; % only bex distortiontype is used
distortion = 0; 
amp = [1, 2, 5];
freq = [4];
...
datafilename = fullfile(dataPath, strcat('practice_unflanked_bex_sub_',subj,'_session_',num2str(session),'.csv')); % name of data file to write to
