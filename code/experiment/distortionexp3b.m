% Function to run a flanked undistorted letter detection task with 4
% distorted flankers and all non-targets being distorted. 
% One frequency and 7 amplitudes are tested.
% 
% params: 
...
if distflankers == 2
    imPath = fullfile(filepathToHere, '../stimuli/exp3bimg2flankersdistorted/distorted');
    fclose('all')
    error('This option is not implemented so far. Please generate images first.')
elseif distflankers == 4
    imPath = fullfile(filepathToHere, '../stimuli/exp3bimg4flankersdistorted/distorted');
elseif distflankers == 0
    imPath = fullfile(filepathToHere, '../stimuli/exp3bimg0flankersdistorted/distorted');
    fclose('all')
    error('This option is not implemented so far. Please generate images first.')
else
    fclose('all')
...
datafilename = fullfile(dataPath, strcat(num2str(distflankers), 'exp3b_', 'distflanker_distortionData_', flankedtype, '_', distortiontype, '_sub_',subj,'_session_',num2str(session),'.csv')); % name of data file to write to
