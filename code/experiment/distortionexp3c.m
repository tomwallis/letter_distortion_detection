% Function to run a flanked letter distortion detection task with either 
% 0,2 or 4 distorted flankers at a fixed high amplitude, while the distortion 
% amplitude of the targets varies. One frequency and 7 amplitudes are tested.
% 
% params: 
% - subject code:   identification of the observer 
% - number of distorted flankers: 0,2,4 flankers are distorted 
%   to the distorted target (images for testing 0,2 distorted flankers have
...
function distortionexp3c()
%% Parameters
session = 1;
lab = true; %run in the lab (true) or on a PC (false)
forcedbreak = 70; 
...
distflankers = input('Enter number of distorted flankers (0, 2 or 4):');
if distflankers == 2
    imPath = fullfile(filepathToHere, '../stimuli/exp3cimg4flankerdistorted/distorted');
    fclose('all')
    error('This option is not implemented so far. Please generate images first.')
elseif distflankers == 4
    imPath = fullfile(filepathToHere, '../stimuli/exp3cimg4flankerdistorted/distorted');
elseif distflankers == 0
    imPath = fullfile(filepathToHere, '../stimuli/exp3cimg4flankerdistorted/distorted');
    fclose('all')
    error('This option is not implemented so far. Please generate images first.')
else
    fclose('all')
    error('Can only distort 0, 2 or 4 flankers')
end
flankedtype = 'flanked';
...
elseif strcmp(distortiontype, 'rf')
    distortion = 1;
    amp = [0.025, 0.05, 0.125, 0.2, 0.275, 0.35, 0.425];
    freq = [4];
    flankeramp=0.425;
else
    fclose('all')
...
% Number of images with same frequency and amplitude
images = 0:9; 

% Number of repititions of the same stimuli 
reps = 3; 

n_amplitudes = length(amp);
n_trials = n_amplitudes*length(images)*reps*length(freq); % number of trials

datafilename = fullfile(dataPath, strcat(num2str(distflankers), 'exp3c_', 'distflanker_distortionData_', flankedtype, '_', distortiontype, '_sub_',subj,'_session_',num2str(session),'.csv')); % name of data file to write to
...
if lab == false
    ...
    %% Create texture for fixationcross
    filename = fullfile(filepathToHere, '../stimuli/fixationcross.png');
...
try
    for trial=1:n_trials
        ...
        % Find image with given flankedtype, distortiontype, freq and amplitude
        ...
        % On a PC (only tested on Windows)       
            ...
            % Show fixationcross
            while (GetSecs - time)<=ms_iti/1000 % change to seconds
            end
