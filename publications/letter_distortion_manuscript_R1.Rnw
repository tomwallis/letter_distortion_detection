...
Word count: 5950 (excluding references and captions). \\
Number of figures: 5 (main text), 11 (supplementary material).
}
...
<<load_clutter_data, echo=FALSE, cache=TRUE>>=
...
cap <- paste0("Clutter analysis for Experiment 1, flanked condition at the highest
...
plt_dat_2 <- clutter_dat_1 %>%
  ...
  filter(amplitude == 0.32) %>%
  group_by(log_freq, flanked, distortion, measure, amplitude)

plt2 <- ggplot(plt_dat_2, aes(x = log_freq, y = clutter)) +
  facet_grid(measure ~ distortion, scales = "free_y") +
  geom_point(position = position_jitter(width=0.1), alpha = 0.5,
             size = image_sizes) +
  stat_summary(fun.data = "mean_se", geom = "line") +
...
cap <- paste0("Clutter analysis of Experiment 2a.
...
plt1 <- ggplot(plt_dat_1, aes(x = n_dist_flanks, y = clutter)) +
  facet_grid(measure ~ distortion, scales = "free_y") +
...
plt2 <- ggplot(plt_dat_2, aes(x = n_dist_flanks, y = clutter)) +
  facet_grid(measure ~ distortion, scales = "free_y") +
  geom_point(position = position_jitter(width=0.2), alpha = 0.5,
             size = image_sizes) +
  stat_summary(fun.data = "mean_se", geom = "line") +
